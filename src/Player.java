...
public class Player extends Rectangle {
    ...
    public static synchronized void updateSprite() {
        ...
        if(goDown||goRight||goLeft||goUp) {
            ...
            if (Main.mouseY < Main.fCenterY) {
                if (Main.mouseX <Main.fWidth/3) {
                    sprite = idleBackSide[idleFrame];
                    flip = true;
                }
                else if (Main.mouseX < Main.fWidth/3+Main.fWidth/3) {
                    sprite = idleBack[idleFrame];
                    if (Main.mouseX < Main.fCenterX)
                        flip = true;
                    else
                        flip = false;
                ...
                if (Main.mouseX <Main.fWidth * 0.4) {
                    sprite = idleSide[idleFrame];
                    flip = true;
                }
                else if (Main.mouseX < Main.fWidth * 0.6) {
                    sprite = idleFront[idleFrame];
                    if (Main.mouseX < Main.fCenterX)
                        flip = true;
                    else
                        flip = false;
        ...
        for (int i = 0; i < 25; i++) {
            for (int j = 0; j < 25; j++) {
                if (Game.room.tileArray[i][j].wall) {
                    ...
                    Point p1 = new Point(x-speed-1, y);
                    if (t.contains(p1)) {
                        colLeft = true;
                    }
                    Point p2 = new Point(x-speed-1, y + height);
                    if (t.contains(p2)) {
                        colLeft = true;
                    }

                    Point p3 = new Point(x+width+speed+1, y);
                    if (t.contains(p3)) {
                        colRight = true;
                    }
                    Point p4 = new Point(x+width+speed+1, y + height);
                    if (t.contains(p4)){
                        colRight = true;
                    }

                    Point p5 = new Point(x, y-speed-1);
                    if (t.contains(p5)){
                        colUp = true;
                    }
                    Point p6 = new Point(x, y-speed-1);
                    if (t.contains(p6)){
                        colUp = true;
                    }

                    Point p7 = new Point(x, y+height+speed+1);
                    if (t.contains(p7)){
                        colDown = true;
                    }
                    Point p8 = new Point(x+width, y + height+speed+1);
                    if (t.contains(p8)){
                        colDown = true;
                    }
                }
